...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from: 'KSPS Finance <no-reply@kingdomscholars.com.ng>',
      to: ['kingdomscholars2@gmail.com'],
      reply_to: email,
      subject: 'New Payment Inquiry — KSPS',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #1B2A4A;">
          <div style="background: #1B2A4A; padding: 24px 32px; border-radius: 8px 8px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 20px;">New Payment Inquiry</h1>
          ...
          <div style="background: white; padding: 32px; border: 1px solid #F5F0E8; border-top: none; border-radius: 0 0 8px 8px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                <td style="padding: 10px 0; border-bottom: 1px solid #F5F0E8; color: #666; font-size: 13px;">Class Interested In</td>
                <td style="padding: 10px 0; border-bottom: 1px solid #F5F0E8; font-size: 14px; font-weight: 600; color: #722F37;">${classInterestedIn}</td>
              </tr>
              <tr>
            ...
            <p style="margin-top: 24px; font-size: 12px; color: #999;">
              This inquiry was submitted via the KSPS website payment inquiry form. Reply directly to this email to respond.
            </p>
          </div>
        </div>
      `,
    ...
    console.error('Payment inquiry route error:', err)
